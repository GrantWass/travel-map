...
export const READABLE_INPUT_CLASS = "bg-white text-stone-900 placeholder:text-stone-500";
export const READABLE_TEXTAREA_CLASS = "bg-white text-stone-900 placeholder:text-stone-500";
...
export function StopEditorCard({
    ...
    return (
        <div id={`stop-${kind}-${stop.id}`} className="rounded-xl border border-stone-200 bg-stone-50/80 p-4">
...
export function StopEditorSection({
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-stone-200 bg-white p-4">
      ...
      {stops.length === 0 ? <p className="text-sm text-stone-500">{config.emptyMessage}</p> : null}
  ...
  return (
    <div>
      ...
      {previewStops.length > 0 ? (
        ...
        <p className="mt-1 text-stone-500">{config.emptyPreviewMessage}</p>
      )}
    </div>
  );
}
